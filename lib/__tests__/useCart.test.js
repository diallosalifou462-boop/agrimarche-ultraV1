...
const makeProduct = (overrides = {}) => (Object.assign({ id: 'prod-1', name: 'Tomates fraîches', description: 'Tomates locales du Sénégal', price: 1000, images: [], category: 'légumes', categoryId: 'cat-1', sellerId: 'seller-1', sellerName: 'Mamadou', sellerRating: 4.5, stock: 50, unit: 'kg', tags: [], rating: 4.5, reviewCount: 10, isOrganic: false, location: 'Dakar', status: 'active', createdAt: new Date(), updatedAt: new Date() }, overrides));
// ─── Tests ────────────────────────────────────────────────────────────────────
describe('useCart', () => {
    beforeEach(() => {
    ...
    it('démarre avec un panier vide', () => {
        const { result } = (0, react_1.renderHook)(() => (0, useCart_1.useCart)());
    ...
    it('cumule la quantité si le produit est déjà dans le panier', () => {
        const { result } = (0, react_1.renderHook)(() => (0, useCart_1.useCart)());
        const product = makeProduct();
        (0, react_1.act)(() => { result.current.addToCart(product, 1); });
        (0, react_1.act)(() => { result.current.addToCart(product, 2); });
        expect(result.current.cart.items).toHaveLength(1);
        expect(result.current.cart.items[0].quantity).toBe(3);
    });
    it('supprime un produit du panier', () => {
        const { result } = (0, react_1.renderHook)(() => (0, useCart_1.useCart)());
        const product = makeProduct();
        (0, react_1.act)(() => { result.current.addToCart(product); });
        (0, react_1.act)(() => { result.current.removeFromCart(product.id); });
        expect(result.current.cart.items).toHaveLength(0);
        expect(result.current.cart.total).toBe(0);
    });
    it('met à jour la quantité', () => {
        const { result } = (0, react_1.renderHook)(() => (0, useCart_1.useCart)());
        const product = makeProduct();
        (0, react_1.act)(() => { result.current.addToCart(product, 5); });
        (0, react_1.act)(() => { result.current.updateQuantity(product.id, 2); });
        expect(result.current.cart.items[0].quantity).toBe(2);
        expect(result.current.cart.total).toBe(2000);
    });
    it('supprime le produit si quantité = 0', () => {
        const { result } = (0, react_1.renderHook)(() => (0, useCart_1.useCart)());
        const product = makeProduct();
        (0, react_1.act)(() => { result.current.addToCart(product, 3); });
        (0, react_1.act)(() => { result.current.updateQuantity(product.id, 0); });
        expect(result.current.cart.items).toHaveLength(0);
    });
    it('vide le panier', () => {
        const { result } = (0, react_1.renderHook)(() => (0, useCart_1.useCart)());
        ...
        (0, react_1.act)(() => { result.current.clearCart(); });
        expect(result.current.cart.items).toHaveLength(0);
        expect(result.current.cart.total).toBe(0);
    });
