...
exports.default = SellerEarningsPage;
...
function SellerEarningsPage() {
    ...
    const { earnings, loading } = (0, useSellerData_1.useSellerData)();
    ...
    return (<div className="max-w-3xl mx-auto px-4 py-6 space-y-5">
      ...
      <div className="bg-gradient-to-br from-green-600 to-emerald-700 rounded-2xl p-6 text-white">
        ...
        {earnings.change !== 0 && (<p className={`text-sm mt-2 font-medium ${earnings.change >= 0 ? 'text-green-200' : 'text-red-200'}`}>
            {earnings.change >= 0 ? '↑' : '↓'} {Math.abs(earnings.change)}% ce mois
          </p>)}
      </div>

      {/* Graphique des revenus */}
      {earnings.history.length > 0 ? (<div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
          ...
          <div className="flex items-end gap-2 h-32">
            {earnings.history.map((h, i) => {
                ...
                return (<div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full bg-green-500 rounded-t-lg transition-all" style={{ height: `${Math.max(pct, 2)}%` }} title={formatPrice(h.amount)}/>
        ...
        </div>) : (<div className="bg-white rounded-2xl p-8 border border-gray-100 shadow-sm text-center">
