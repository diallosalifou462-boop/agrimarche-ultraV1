...
function playNotificationSound(type = "default", enabled = true) {
    if (!enabled)
        return;
    try {
...
function NotificationSystem() {
    ...
    const [activeTab, setActiveTab] = (0, react_1.useState)("inbox");
    const [soundEnabled, setSoundEnabled] = (0, react_1.useState)(true);
    const [pushEnabled, setPushEnabled] = (0, react_1.useState)(false);
    const [emailFallback, setEmailFallback] = (0, react_1.useState)(true);
    const [deepLinkEnabled, setDeepLinkEnabled] = (0, react_1.useState)(true);
    const [selectedNotif, setSelectedNotif] = (0, react_1.useState)(null);
    const [aiAnalysis, setAiAnalysis] = (0, react_1.useState)(null);
    const [aiLoading, setAiLoading] = (0, react_1.useState)(false);
    const [toastQueue, setToastQueue] = (0, react_1.useState)([]);
    const [filterType, setFilterType] = (0, react_1.useState)("all");
    const [pushStatus, setPushStatus] = (0, react_1.useState)("idle");
    const [emailInput, setEmailInput] = (0, react_1.useState)((profile === null || profile === void 0 ? void 0 : profile.email) || "");
    // ── Firestore real-time listener ──
    (0, react_1.useEffect)(() => {
        if (!(profile === null || profile === void 0 ? void 0 : profile.uid))
            return;
        const q = (0, firestore_1.query)((0, firestore_1.collection)(firebase_1.db, "notifications"), (0, firestore_1.where)("userId", "==", profile.uid), (0, firestore_1.orderBy)("createdAt", "desc"));
            ...
            if (!loading) {
                snap.docChanges().forEach((change) => {
                    if (change.type === "added") {
                        const n = Object.assign({ id: change.doc.id }, change.doc.data());
                        if (!n.read)
                            showToast(n);
                    }
                });
        ...
        return () => unsub();
    }, [profile === null || profile === void 0 ? void 0 : profile.uid]); // eslint-disable-line
    // ── Toast ──
    const showToast = (0, react_1.useCallback)((notif) => {
        const t = {
            toastId: Date.now(),
            type: notif.type,
            title: notif.title,
    ...
    const markRead = async (notif) => {
        if (!notif.id || notif.read)
            return;
        await (0, firestore_1.updateDoc)((0, firestore_1.doc)(firebase_1.db, "notifications", notif.id), { read: true });
    };
    // ── Mark all read ──
    const markAllRead = async () => {
        ...
        notifications
            ...
            .forEach((n) => batch.update((0, firestore_1.doc)(firebase_1.db, "notifications", n.id), { read: true }));
        await batch.commit();
    };
    ...
    const simulateIncoming = async () => {
        ...
        await (0, firestore_1.addDoc)((0, firestore_1.collection)(firebase_1.db, "notifications"), Object.assign(Object.assign({}, tpl), { userId: profile.uid, read: false, createdAt: firestore_1.Timestamp.now() }));
    };
    ...
    const requestPush = async () => {
        ...
        if ("Notification" in window) {
            ...
            if (perm === "granted") {
                ...
                if (emailFallback) {
                    const fallbackNotif = {
                        userId: (profile === null || profile === void 0 ? void 0 : profile.uid) || "",
                        type: "alert",
                        title: "Push refusé",
    ...
    const runAiAnalysis = async () => {
        ...
        const type = (selectedNotif === null || selectedNotif === void 0 ? void 0 : selectedNotif.type) || "general";
        setAiAnalysis(AI_ANALYSES[type] || AI_ANALYSES.general);
        setAiLoading(false);
    };
    // ── Deep link handler ──
    const handleDeepLink = (notif) => {
        markRead(notif);
        setSelectedNotif(notif);
        if (deepLinkEnabled) {
            const t = { toastId: Date.now(), type: "default", title: "Deep link", body: `Navigation → ${notif.deepLink}`, icon: "🔗", urgent: false };
            setToastQueue((q) => [...q, t]);
            setTimeout(() => setToastQueue((q) => q.slice(1)), 4000);
        }
    };
    ...
    // ─── RENDER ────────────────────────────────────────────────────────────────
    return (<div style={{ minHeight: "100vh", background: "linear-gradient(145deg,#0a0806 0%,#12100c 40%,#0f0d0a 100%)", fontFamily: "'DM Sans',sans-serif", color: "#e8d5a3", display: "flex", flexDirection: "column", alignItems: "center", padding: "0 0 40px", position: "relative", overflow: "hidden" }}>
      <style>{`
      ...
      <div style={{ width: "100%", maxWidth: 540, padding: "32px 20px 0", position: "relative", zIndex: 1 }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24 }}>
          <div>
            ...
            {unread > 0 && (<div style={{ marginTop: 6, display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(212,168,83,0.12)", border: "1px solid rgba(212,168,83,0.25)", borderRadius: 20, padding: "3px 10px" }}>
      ...
      <div style={{ width: "100%", maxWidth: 540, padding: "16px 20px", position: "relative", zIndex: 1 }}>
        ...
        {/* ── INBOX ── */}
        {activeTab === "inbox" && (<div style={{ animation: "fadeIn 0.3s ease" }}>
            {/* Filter chips */}
            <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 8, marginBottom: 12, scrollbarWidth: "none" }}>
              {["all", "order", "message", "price", "delivery", "alert"].map((f) => (<button key={f} onClick={() => setFilterType(f)} style={{ flexShrink: 0, padding: "4px 12px", borderRadius: 20, border: `1px solid ${filterType === f ? "#d4a853" : "rgba(255,255,255,0.1)"}`, background: filterType === f ? "rgba(212,168,83,0.15)" : "transparent", color: filterType === f ? "#d4a853" : "rgba(232,213,163,0.5)", fontSize: 11, cursor: "pointer", fontWeight: 500, transition: "all 0.2s" }}>
                  {f === "all" ? "Tout" : f}{f !== "all" && grouped[f] ? ` (${grouped[f]})` : ""}
                </button>))}
            </div>

            {unread > 0 && (<button onClick={markAllRead} style={{ width: "100%", padding: "8px", marginBottom: 12, background: "transparent", border: "1px solid rgba(212,168,83,0.2)", borderRadius: 8, color: "rgba(212,168,83,0.7)", fontSize: 11, cursor: "pointer", fontFamily: "'DM Sans'" }}>
                Tout marquer comme lu
              </button>)}

            {/* Loading */}
            {loading && (<div style={{ textAlign: "center", padding: 40, color: "rgba(212,168,83,0.5)" }}>
            ...
            {/* AI Panel */}
            {selectedNotif && (<div style={{ marginTop: 16, padding: 16, borderRadius: 14, border: "1px solid rgba(212,168,83,0.2)", background: "rgba(212,168,83,0.05)", animation: "fadeIn 0.3s ease" }}>
                ...
                {aiLoading ? (<div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                  ...
                  </div>) : aiAnalysis ? (<p style={{ margin: 0, fontSize: 12, color: "#d4a853", lineHeight: 1.6 }}>{aiAnalysis}</p>) : (<button onClick={runAiAnalysis} className="action-btn" style={{ background: "rgba(212,168,83,0.12)", border: "1px solid rgba(212,168,83,0.3)", color: "#d4a853", borderRadius: 8, padding: "8px 16px", cursor: "pointer", fontSize: 12, fontWeight: 500 }}>
                    Analyser avec l'IA
                  </button>)}
              </div>)}
          </div>)}
        ...
        {activeTab === "dashboard" && (<div style={{ animation: "fadeIn 0.3s ease" }}>
            ...
            ].map((k, i) => (<div key={i} style={{ padding: 16, borderRadius: 14, border: "1px solid rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.03)", animation: `fadeDown 0.4s ease ${i * 0.08}s both` }}>
            ...
            <div style={{ padding: 16, borderRadius: 14, border: "1px solid rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.02)", marginBottom: 16 }}>
              ...
              {Object.entries(grouped).map(([type, count], i) => (<div key={type} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10, animation: `fadeDown 0.4s ease ${i * 0.06}s both` }}>
            ...
            ].map((ch, i) => (<div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
        ...
        {/* ── SETTINGS ── */}
        {activeTab === "settings" && (<div style={{ animation: "fadeIn 0.3s ease", display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            {[
                { key: "sound", label: "Son de notification", desc: "Joue un son à chaque notification reçue", icon: "🔊", value: soundEnabled, set: (v) => { setSoundEnabled(v); if (v)
            ...
            ].map((s) => (<div key={s.key} style={{ padding: 16, borderRadius: 14, border: "1px solid rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.02)" }}>
            ...
            <div style={{ padding: 16, borderRadius: 14, border: `1px solid ${pushEnabled ? "rgba(106,184,160,0.3)" : "rgba(212,168,83,0.2)"}`, background: pushEnabled ? "rgba(106,184,160,0.05)" : "rgba(212,168,83,0.04)" }}>
              ...
              {pushStatus === "granted" || pushEnabled ? (<div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", borderRadius: 8, background: "rgba(106,184,160,0.1)", border: "1px solid rgba(106,184,160,0.2)" }}>
                ...
                </div>) : pushStatus === "denied" ? (<div style={{ fontSize: 12, color: "#e05454", padding: "8px 12px", borderRadius: 8, background: "rgba(224,84,84,0.1)", border: "1px solid rgba(224,84,84,0.2)" }}>
                  ⛔ Permission refusée — email de repli {emailFallback ? "activé" : "désactivé"}
                </div>) : (<button onClick={requestPush} className="action-btn" style={{ width: "100%", padding: "10px", borderRadius: 10, border: "1px solid rgba(212,168,83,0.4)", background: "rgba(212,168,83,0.1)", color: "#d4a853", cursor: "pointer", fontSize: 13, fontWeight: 500 }}>
                  {pushStatus === "requesting" ? "⏳ Demande en cours…" : "Autoriser les notifications push"}
                </button>)}
            </div>

            {/* Email config */}
            {emailFallback && (<div style={{ padding: 16, borderRadius: 14, border: "1px solid rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.02)", animation: "fadeDown 0.3s ease" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {Object.keys(NOTIFICATION_SOUNDS).map((type) => (<button key={type} onClick={() => playNotificationSound(type, true)} className="action-btn" style={{ padding: "10px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.03)", color: "rgba(232,213,163,0.6)", cursor: "pointer", fontSize: 12 }}>
                  ▶ Son {type}
                </button>))}
            </div>
          </div>)}
      ...
      <div style={{ position: "fixed", top: 20, right: 16, zIndex: 1000, display: "flex", flexDirection: "column", gap: 8, maxWidth: 300 }}>
        {toastQueue.map((t) => (<div key={t.toastId} style={{ padding: "12px 14px", borderRadius: 12, border: `1px solid ${t.urgent ? "rgba(224,84,84,0.4)" : "rgba(212,168,83,0.25)"}`, background: "rgba(10,8,6,0.95)", backdropFilter: "blur(12px)", animation: "toastIn 0.3s ease", boxShadow: "0 4px 24px rgba(0,0,0,0.6)", display: "flex", gap: 10, alignItems: "flex-start" }}>
