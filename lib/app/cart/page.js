...
function CartPage() {
    ...
    const { cart, removeFromCart, updateQuantity, clearCart } = (0, useCart_1.useCart)();
    ...
    if (cart.items.length === 0) {
        return (<div className="min-h-screen bg-gradient-to-br from-white via-gray-50 to-emerald-50/20 flex flex-col items-center justify-center p-6 text-center">
        ...
        <button onClick={() => router.push('/main/products')} className="px-12 py-5 bg-gradient-to-r from-emerald-600 to-green-600 text-white font-light text-lg rounded-full transition-all duration-500 hover:shadow-2xl hover:shadow-emerald-500/25 hover:scale-105">
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-white via-gray-50 to-emerald-50/10">
      {/* Header */}
      <header className="sticky top-0 z-50 bg-white/60 backdrop-blur-xl border-b border-emerald-100">
        <div className="max-w-2xl mx-auto px-4 py-4">
          <div className="flex items-center gap-3">
            <button onClick={() => router.back()} className="w-10 h-10 rounded-full bg-white/80 hover:bg-emerald-50 transition-all duration-500 flex items-center justify-center shadow-sm">
            ...
            <button onClick={clearCart} className="w-10 h-10 rounded-full bg-white/80 hover:bg-rose-50 transition-all duration-500 flex items-center justify-center shadow-sm">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-5 pb-32">
        ...
        {cart.items.map((item, idx) => {
            var _a;
            const images = (_a = item.product.images) !== null && _a !== void 0 ? _a : [];
            const productValue = item.product.price * item.quantity;
            const isLast = idx === cart.items.length - 1;
            return (<div key={item.product.id} className="group bg-white rounded-2xl p-5 shadow-lg hover:shadow-xl transition-all duration-500 border border-emerald-100/50 hover:border-emerald-200" style={{ animationDelay: `${idx * 100}ms` }}>
              <div className="flex gap-5">
                ...
                <div className="relative w-24 h-24 flex-shrink-0">
                  ...
                  <div className="relative w-full h-full bg-gradient-to-br from-emerald-50 to-green-50 rounded-xl overflow-hidden shadow-md">
                    {images[0] ? (<img src={images[0]} alt={item.product.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"/>) : (<div className="w-full h-full flex items-center justify-center text-3xl">
                        🌾
                      </div>)}
                  </div>
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[8px] text-gray-400 mt-1">
                    ...
                    {item.product.farmerVerified && (<span className="ml-2 text-[6px] bg-emerald-100 text-emerald-600 px-1.5 py-0.5 rounded-full">Vérifié</span>)}
                  </p>
                  <div className="mt-2">
                  ...
                  <div className="flex items-center gap-3 mt-3">
                    <div className="flex items-center gap-1 bg-gray-50 rounded-full p-1">
                      <button onClick={() => updateQuantity(item.product.id, item.quantity - 1)} className="w-7 h-7 rounded-full bg-white hover:bg-emerald-500 hover:text-white flex items-center justify-center text-sm transition-all duration-300 shadow-sm">
                        −
                      </button>
                      <span className="text-sm font-medium text-gray-700 w-8 text-center">{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.product.id, item.quantity + 1)} disabled={item.quantity >= (item.product.stock || 999)} className="w-7 h-7 rounded-full bg-white hover:bg-emerald-500 hover:text-white flex items-center justify-center text-sm transition-all duration-300 shadow-sm disabled:opacity-40">
                        +
                      </button>
                    </div>
                    {item.product.stock && item.product.stock > 0 && item.product.stock <= 10 && (<span className="text-[6px] bg-amber-100 text-amber-600 px-2 py-1 rounded-full">Stock limité</span>)}
                  </div>
                </div>

                {/* Valeur */}
                <div className="flex flex-col items-end justify-between flex-shrink-0">
                  <button onClick={() => removeFromCart(item.product.id)} className="text-gray-300 hover:text-rose-500 transition-all duration-300 text-xl w-8 h-8 rounded-full hover:bg-rose-50 flex items-center justify-center">
                    ×
                  </button>
              ...
              {!isLast && (<div className="mt-5 h-px bg-gradient-to-r from-transparent via-emerald-200 to-transparent"/>)}
            </div>);
        })}
        ...
        <div className="relative mt-8">
          <div className="relative bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-emerald-100">
            ...
            <button onClick={handleCheckout} className="mt-6 w-full bg-gradient-to-r from-emerald-600 to-green-600 text-white font-light py-4 rounded-full transition-all duration-500 hover:shadow-xl hover:shadow-emerald-500/25 hover:scale-[1.02]">
              Passer la commande
            </button>

            <button onClick={() => router.push('/main/products')} className="mt-3 w-full text-center text-xs text-gray-400 hover:text-emerald-600 py-2 transition-colors duration-300">
