...
function MainPage() {
    ...
    (0, react_1.useEffect)(() => {
        const generatedParticles = [...Array(55)].map((_, i) => ({
            id: i,
    ...
    return (<div className="sacred-container">
      <style jsx global>{`
      ...
      <div className="celestial-breath" onMouseMove={(e) => {
            document.documentElement.style.setProperty('--x', e.clientX + 'px');
            document.documentElement.style.setProperty('--y', e.clientY + 'px');
        }}/>
      <div className="eternal-silence"/>

      {particles.map((particle) => (<div key={particle.id} className="light-seed" style={{
                left: particle.left,
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-6 py-12">
        <div className="emblem-glow mb-10">
          <div className="sacred-emblem rounded-3xl shadow-2xl">
            ...
            <image_1.default src="/logo.png" alt="Agrimarché" width={300} height={300} priority className="rounded-3xl"/>
          </div>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-6 mt-16">
          ...
          {/* 🔓 PRODUITS - GRATUIT */}
          <link_1.default href="/main/products" className="gate-btn bg-gradient-to-r from-emerald-800 to-emerald-600 hover:from-emerald-700 hover:to-emerald-500 text-white px-8 md:px-12 py-4 rounded-full font-bold shadow-2xl transition-all duration-300 text-center tracking-wide" onMouseMove={(e) => {
          ...
          {/* 🔒 IA PREMIUM */}
          {checking ? (<div className="gate-btn bg-gradient-to-r from-purple-400 to-indigo-400 text-white px-8 md:px-12 py-4 rounded-full font-bold opacity-60 cursor-wait text-center tracking-wide">
              ⏳ Vérification…
            </div>) : hasAIAccess ? (<link_1.default href="/main/ai-assistant" className="gate-btn bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white px-8 md:px-12 py-4 rounded-full font-bold shadow-2xl transition-all duration-300 text-center tracking-wide flex items-center gap-2" onMouseMove={(e) => {
                const rect = e.currentTarget.getBoundingClientRect();
                e.currentTarget.style.setProperty('--x', (e.clientX - rect.left) + 'px');
                e.currentTarget.style.setProperty('--y', (e.clientY - rect.top) + 'px');
            }}>
            ...
            </link_1.default>) : (<link_1.default href="/" className="ia-premium-btn text-white px-8 md:px-12 py-4 rounded-full font-bold transition-all duration-300 text-center tracking-wide relative inline-flex items-center justify-center" onMouseMove={(e) => {
                const rect = e.currentTarget.getBoundingClientRect();
                e.currentTarget.style.setProperty('--x', (e.clientX - rect.left) + 'px');
                e.currentTarget.style.setProperty('--y', (e.clientY - rect.top) + 'px');
            }}>
          ...
          {/* 🔓 PROFIL - GRATUIT */}
          <link_1.default href="/main/account" className="gate-outline border-2 border-emerald-500 text-emerald-700 bg-white/30 px-8 md:px-12 py-4 rounded-full font-bold transition-all duration-300 text-center shadow-md hover:shadow-xl">
            👤 mon profil
          </link_1.default>
        </div>
