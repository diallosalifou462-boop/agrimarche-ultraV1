...
function AgriMarket() {
    ...
    (0, react_1.useEffect)(() => {
        ...
        voiceRef.current.onresult = (e) => { setSearch(e.results[0][0].transcript); setListening(false); };
        voiceRef.current.onerror = () => setListening(false);
        voiceRef.current.onend = () => setListening(false);
    }, []);
    const startVoice = () => { if (voiceRef.current && !listening) {
    ...
    return (<>
      ...
      <header className={`g-header ${scrolled ? 'scrolled' : ''}`}>
        <div className="g-header-inner">
          <div className="g-logo-row">
            <link_1.default href="/" className="g-logo-link">
            ...
            <div className="g-header-right">
              <link_1.default href="/cart" className="g-icon-btn">
                🛒
              ...
              <div style={{ position: 'relative' }}>
                ...
                {showUserMenu && (<div className="g-user-menu">
                    <div className="g-um-head">
                      <div className="g-um-role">COMPTE</div>
                      <div className="g-um-email">{(user === null || user === void 0 ? void 0 : user.email) || 'Invité'}</div>
                    </div>
                    <link_1.default href="/privacy" className="g-um-item">🛡️ &nbsp;Confidentialité</link_1.default>
                    <link_1.default href="/account" className="g-um-item">👤 &nbsp;Mon compte</link_1.default>
                    {user && (<button onClick={async () => { await logout(); router.push('/'); }} className="g-um-item red">
                        🚪 &nbsp;Déconnexion
                      </button>)}
                  </div>)}
              </div>
          ...
          <div className="g-search-row">
            <div className="g-search-box">
              ...
              <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Tomates Casamance, mil Thiès, oignons Potou…" className="g-search-input"/>
      ...
      <div className="g-cats">
        <div className="g-cats-inner">
          {CATEGORIES.map(c => (<button key={c.label} onClick={() => setCat(c.label)} className={`g-cat ${cat === c.label ? 'on' : ''}`}>
      ...
      <div className="g-toolbar">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
          <button className="g-loc-chip" onClick={getLocation} style={{ minWidth: 0 }}>
            ...
            locStatus === 'error' ? 'Non disponible' :
                (location === null || location === void 0 ? void 0 : location.detailedAddress) || (location === null || location === void 0 ? void 0 : location.address) || 'Sénégal'}
            </span>
          </button>
        ...
        <div style={{ position: 'relative', flexShrink: 0 }}>
          ...
          {showSort && (<div className="g-sort-menu">
              {['default', 'asc', 'desc'].map(m => (<button key={m} onClick={() => { setSort(m); setShowSort(false); }} className={`g-sort-item ${sort === m ? 'on' : ''}`}>
      ...
      <main className="g-main">
        ...
        <div className="g-grid">
          {filtered.length === 0 ? (<div className="g-empty">
            ...
            </div>) : (filtered.map((p, idx) => {
            var _a, _b, _c;
            const rd = ratings.get(p.sellerId || '');
            const stars = (rd === null || rd === void 0 ? void 0 : rd.averageRating) || 0;
            const cnt = (rd === null || rd === void 0 ? void 0 : rd.reviewCount) || 0;
            const isHero = idx % 9 === 0 && idx !== 0;
            return (<div key={p.id} className={`g-card ${isHero ? 'hero' : ''}`} style={{ animationDelay: `${Math.min(idx * 38, 700)}ms` }} onClick={() => open(p)}>
                  <div className="g-card-visual">
                    {((_a = p.images) === null || _a === void 0 ? void 0 : _a[0]) ? (<image_1.default src={p.images[0]} alt={p.name} fill style={{ objectFit: 'cover', transition: 'transform 0.6s cubic-bezier(.16,1,.3,1)' }}/>) : (<div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 52, background: 'var(--graphite)' }}>
                        🌾
                      </div>)}
                    ...
                    <div className="g-card-price">
                      <div>
                        <div className="g-price-n">{(_b = p.price) === null || _b === void 0 ? void 0 : _b.toLocaleString()}</div>
                  ...
                  <div className="g-card-body">
                    ...
                    <div className="g-card-farmer">
                      ...
                      <span className="g-farmer-name">{((_c = p.farmer) === null || _c === void 0 ? void 0 : _c.split(' ')[0]) || 'Producteur'}</span>
                    </div>
                    {cnt > 0 && (<div className="g-stars">
                        {[1, 2, 3, 4, 5].map(i => (<svg key={i} width={10} height={10} viewBox="0 0 24 24" fill={i <= Math.floor(stars) ? 'var(--jade)' : 'rgba(13,74,31,0.15)'}>
      ...
      <nav className="g-nav">
        <div className="g-nav-inner">
          <link_1.default href="/" className="g-nav-btn on">
          ...
          <button className="g-nav-cta" onClick={() => window.open(`https://wa.me/${WA_NUMBER}?text=${encodeURIComponent('Bonjour, je souhaite commander sur AgriMarché.')}`, '_blank')}>
            <div className="g-nav-cta-ring"><WaIcon s={26}/></div>
            <span className="g-nav-cta-lbl">WHATSAPP</span>
          </button>

          <link_1.default href="/cart" className="g-nav-btn">
            <span className="g-nav-ic">◻</span>
            <span className="g-nav-lbl">PANIER</span>
          </link_1.default>
          <link_1.default href="/account" className="g-nav-btn">
            <span className="g-nav-ic">◎</span>
      ...
      {selected && (<>
          ...
          <div className="g-drawer" ref={drawerRef}>
            ...
            <div className="g-drawer-gallery">
              {((_a = selected.images) === null || _a === void 0 ? void 0 : _a[imgIdx]) ? (<image_1.default src={selected.images[imgIdx]} alt={selected.name} fill style={{ objectFit: 'cover' }}/>) : (<div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 80 }}>🌾</div>)}
              {((_b = selected.images) === null || _b === void 0 ? void 0 : _b.length) > 1 && (<div className="g-gallery-dots">
                  {selected.images.map((_, i) => (<button key={i} onClick={() => setImgIdx(i)} className={`g-gdot ${imgIdx === i ? 'on' : ''}`}/>))}
                </div>)}
            </div>
            ...
            <div className="g-dc">
              ...
              <div className="g-dc-price-row">
                <span className="g-dc-price">{(_c = selected.price) === null || _c === void 0 ? void 0 : _c.toLocaleString()}</span>
                <span className="g-dc-unit">FCFA / {selected.unit || 'kg'}</span>
              </div>
              {selected.description && <p className="g-dc-desc">{selected.description}</p>}

              {categoryProducts.length > 0 && (<div className="g-category-cards">
                  ...
                  <div className="g-category-grid">
                    ...
                    var _a, _b;
                    return (<div key={catProd.id} className="g-category-card" onClick={(e) => {
                            var _a;
                            e.stopPropagation();
                            setSelected(catProd);
                            setImgIdx(0);
                            (_a = drawerRef.current) === null || _a === void 0 ? void 0 : _a.scrollTo({ top: 0, behavior: 'smooth' });
                        }}>
                        <div className="g-category-card-image">
                          {((_a = catProd.images) === null || _a === void 0 ? void 0 : _a[0]) ? (<image_1.default src={catProd.images[0]} alt={catProd.name} fill style={{ objectFit: 'cover' }}/>) : (<div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24 }}>🌾</div>)}
                        </div>
                        <div className="g-category-card-cat">{catProd.category}</div>
                        <div className="g-category-card-name">{catProd.name}</div>
                        <div className="g-category-card-price">{(_b = catProd.price) === null || _b === void 0 ? void 0 : _b.toLocaleString()} FCFA</div>
                      </div>);
                })}
                  </div>
                </div>)}
              ...
              <div className="g-dc-meta">
                ...
                {selected.stock !== undefined && (<div className="g-meta-row">
                    <div className="g-meta-icon">📦</div>
            ...
            {recs.length > 0 && (<div className="g-recs">
                ...
                <div className="g-recs-grid">
                  {recs.map(r => {
                    var _a, _b;
                    return (<div key={r.id} className="g-rec" onClick={() => { var _a; setSelected(r); setImgIdx(0); (_a = drawerRef.current) === null || _a === void 0 ? void 0 : _a.scrollTo({ top: 0, behavior: 'smooth' }); }}>
                      <div className="g-rec-img">
                        {((_a = r.images) === null || _a === void 0 ? void 0 : _a[0])
                            ? <image_1.default src={r.images[0]} alt={r.name} width={50} height={50} style={{ objectFit: 'cover', width: '100%', height: '100%' }}/>
                            : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>🌾</div>}
                      </div>
                      <div style={{ minWidth: 0 }}>
                        <div className="g-rec-cat">{r.category}</div>
                        <div className="g-rec-name">{r.name}</div>
                        <div className="g-rec-price">{(_b = r.price) === null || _b === void 0 ? void 0 : _b.toLocaleString()} FCFA</div>
                      </div>
                    </div>);
                })}
                </div>
