...
try {
  cleanNextCache();
  moveOut();
  // --webpack : Turbopack nécessite des bindings natifs (next-swc) qui
  // peuvent être absents/corrompus selon la plateforme/version de Node
  // (ex: Windows + Node très récent sans binaire précompilé disponible).
  // Turbopack retombe alors sur des bindings WASM, qu'il ne supporte pas,
  // et le build échoue avec "Turbopack is not supported on this platform".
  // Webpack n'a pas cette dépendance à des bindings natifs et est donc
  // plus robuste pour ce script cross-plateforme.
