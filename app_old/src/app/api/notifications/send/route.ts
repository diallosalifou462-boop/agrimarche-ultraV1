      ...
      if (deviceTokens.length === 0) {
        ...
        try {
          const multicast = await getMessaging(adminApp).sendEachForMulticast({
            ...
            // ⚠️ FIX : bloc apns manquant. Sans lui, FCM ne délivre pas de
            // façon fiable aux devices iOS (pas de son, pas de bannière
            // quand l'app est en arrière-plan/fermée) — voir /api/send-push
            // qui a déjà ce bloc correctement. C'était la cause du "push OK
            // sur Android, jamais reçu sur iPhone".
            apns: {
              payload: {
                aps: {
                  alert: { title, body },
        ...
        try {
          ...
          if (!infobipBaseUrl || !infobipApiKey) {
            ...
            if (!smsResponse.ok) {
              ...
              // ⚠️ FIX : même correctif que otp/send et send-sms —
              // PENDING_ACCEPTED (groupe "PENDING") = envoi réussi, pas rejet.
              const msgStatus = smsData?.messages?.[0]?.status;
              results.sms = {
                sent:      msgStatus?.groupName !== 'REJECTED',
