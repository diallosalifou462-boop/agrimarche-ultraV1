...
function toE164Digits(phone: string): string {
  ...
  if (!digits.startsWith('221') || digits.length !== 12) {
    throw new HttpsError(
      'invalid-argument',
      'Numéro sénégalais invalide, format attendu +221XXXXXXXXX'
    );
  }
  return digits;
}
...
export const requestWhatsAppOtp = onCall(async (request) => {
  const phoneRaw = request.data?.phone;
  if (typeof phoneRaw !== 'string') {
    throw new HttpsError('invalid-argument', 'Numéro de téléphone manquant');
  }
...
export const verifyWhatsAppOtp = onCall(async (request) => {
  ...
  if (typeof phoneRaw !== 'string' || typeof code !== 'string') {
    throw new HttpsError('invalid-argument', 'Numéro ou code manquant');
  }
  ...
  if (!snap.exists) {
    throw new HttpsError('not-found', 'Aucun code demandé pour ce numéro');
  }
  ...
  if (data.expiresAt.toMillis() < Date.now()) {
    await otpRef.delete();
    throw new HttpsError('deadline-exceeded', 'Code expiré, redemandez-en un');
  }

  if (data.attempts >= MAX_ATTEMPTS) {
    await otpRef.delete();
    throw new HttpsError('resource-exhausted', 'Trop de tentatives, redemandez un code');
  }
