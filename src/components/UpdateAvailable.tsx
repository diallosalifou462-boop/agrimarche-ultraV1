...
export const UpdateAvailable: React.FC = () => {
  ...
  const handleUpdate = () => {
    if ('serviceWorker' in navigator) {
      // ⚠️ FIX : on attend le VRAI événement 'controllerchange' avant de
      // recharger. Avant, reload() était appelé juste après avoir envoyé
      // SKIP_WAITING, sans attendre que le nouveau SW ait réellement pris
      // le contrôle — la page pouvait donc se recharger via l'ANCIEN SW/
      // cache, redétecter la "mise à jour disponible", et reproposer le
      // même cycle indéfiniment.
      let reloaded = false;
