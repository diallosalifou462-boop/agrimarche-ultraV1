...
interface Order {
  ...
  // Ces clés viennent de @/lib/orderStatus — la même source unique que
  // seller-orders-page.tsx, account (page.tsx) et delivery-dashboard-page.tsx,
  // donc elles ne peuvent plus diverger entre les pages.
  // ────────────────────────────────────────────────────────────────────────────
  status: OrderStatus;
  statusLabel: string;
  total: number;
...
// Avant, cette page avait sa propre palette (ex. 'en_preparation' en bleu ici,
// en violet partout ailleurs, 'livre' en 📦 ici et ✅ ailleurs) — un exemple
// concret de la dérive que la source unique élimine.
const TRACKING_STEPS = [
  { key: 'en_attente',     label: 'Commande reçue',           doneWhen: (_s: string) => true },
  { key: 'en_preparation', label: 'Acceptée par le vendeur',  doneWhen: (s: string) => ['en_preparation','en_livraison','livre'].includes(s) },
  { key: 'en_livraison',   label: 'En cours de livraison',    doneWhen: (s: string) => ['en_livraison','livre'].includes(s) },
...
function OrdersContent() {
  ...
  const status = selectedOrder
    ? { ...ORDER_STATUS_CONFIG[selectedOrder.status], bg: statusTint(selectedOrder.status, 0.12) }
    : null;
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAFAF8', fontFamily: 'DM Sans, sans-serif' }}>
      <style>{`
        ...
        .confirm-btn { display:flex; align-items:center; justify-content:center; gap:8px; background:#d1fae5; color:#065f46; border:1px solid #6ee7b7; border-radius:12px; padding:12px 20px; font-size:13px; font-weight:600; cursor:pointer; transition:opacity 0.2s; width:100%; }
      ...
      <div style={{ display: 'flex', maxWidth: 960, margin: '0 auto', minHeight: 'calc(100vh - 80px)' }}>
        ...
        <div style={{ width: 340, flexShrink: 0, borderRight: '1px solid #f0ede8', padding: '16px 12px', overflowY: 'auto' }}>
          {orders.map(order => {
            const cfg = { ...ORDER_STATUS_CONFIG[order.status], bg: statusTint(order.status, 0.12) };
                  ...
                  <div style={{ marginTop: 16, borderTop: '1px solid rgba(201,169,110,0.15)', paddingTop: 16, animation: 'fadeIn 0.25s ease' }}>
                          ...
                          return (
                            <div key={step.key} className="track-step">
                              <div className="track-dot" style={{ background: done ? '#d1fae5' : '#f3f4f6', color: done ? '#065f46' : '#9A9A9A' }}>
                                {ORDER_STATUS_CONFIG[step.key as OrderStatus].icon}
                              </div>
                              <div>
                      ...
                      {selectedOrder.items?.map((item, i) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 0', borderBottom: i < selectedOrder.items.length - 1 ? '1px solid #f3f4f6' : 'none' }}>
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            <p style={{ margin: 0, fontSize: 11, color: '#9A9A9A' }}>{item.quantity} {item.unit} · {formatFCFA(item.productPrice)}/{item.unit}</p>
                          </div>
                          <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: '#1A1A1A' }}>{formatFCFA(item.productPrice * item.quantity)}</p>
                        </div>
                      ))}
                    </div>
                    ...
                    <div style={{ background: '#FAFAF8', borderRadius: 12, padding: '14px 16px', marginBottom: 16, border: '1px solid rgba(201,169,110,0.15)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                        ...
                        <span style={{ fontSize: 12, fontWeight: 600, color: '#065f46' }}>{formatFCFA(selectedOrder.depositAmount)} ✓</span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <span style={{ fontSize: 12, fontWeight: 600, color: selectedOrder.status === 'livre' ? '#9A9A9A' : '#92400e' }}>
                          {selectedOrder.status === 'livre' ? '✓ Réglé' : formatFCFA(selectedOrder.remainingAmount)}
                        </span>
                      </div>
                    ...
                    {/* ✅ NOUVEAU — Livreur assigné : dès qu'un livreur accepte
                        ...
                        prend en charge sa livraison.
                        🔒 FIX SÉCURITÉ : le client ne doit JAMAIS voir le
                        téléphone/WhatsApp direct du livreur (ni l'inverse —
                        voir seller/orders/page.tsx et delivery/dashboard).
                        Tout contact passe par la plateforme ; le seul canal
                        de clôture de la livraison est le code ci-dessous. */}
                    {selectedOrder.delivererName && ['en_livraison', 'livre'].includes(selectedOrder.status) && (
                      <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 12, padding: '12px 14px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      ...
                      {canConfirm && (
                        <button
                          ...
                          onClick={e => { e.stopPropagation(); handleConfirmDelivery(selectedOrder.id); }}
                        >
                          {confirming === selectedOrder.id
                            ? <><span style={{ width: 14, height: 14, border: '2px solid #065f46', borderTopColor: 'transparent', borderRadius: '50%', display: 'inline-block', animation: 'spin 0.8s linear infinite' }} /> Confirmation…</>
