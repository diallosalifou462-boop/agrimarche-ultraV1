...
import { readSellerShopLocation } from '@/lib/geo/userLocation';
import { getBestGpsFix, formatReverseAddress, GPS_PRECISE_ACCURACY_M } from '@/lib/geo/quality';
  ...
  const updateStatus = async (id: string, newStatus: OrderStatus, statusLabel: string) => {
    ...
    try {
      ...
      if (newStatus === 'en_preparation') {
        ...
        // Point de retrait = celui ENREGISTRÉ par le vendeur (GPS, carte ou
        // recherche). La position du téléphone au moment d'accepter n'est
        // utilisée qu'en dernier recours, si aucun point n'est enregistré ET
        // que le GPS est précis — jamais un point à ±2 km.
        try {
          const profileSnap = await getDoc(doc(db, 'users', user.uid));
          const shop = readSellerShopLocation(profileSnap.data());
          if (shop) {
            payload.sellerLocation = {
              ...
              isDefault: false,
              locationSource: shop.source,
              ...(typeof shop.accuracy === 'number' ? { accuracy: shop.accuracy } : {}),
            };
          } else {
            const fix = await getBestGpsFix({ maxWaitMs: 12000 });
            if (fix.accuracy <= GPS_PRECISE_ACCURACY_M) {
              const geocoded = await reverseGeocode(fix.lat, fix.lng).catch(() => null);
              payload.sellerLocation = {
                lat: fix.lat,
                lng: fix.lng,
                address: formatReverseAddress(geocoded, fix.lat, fix.lng),
                isDefault: false,
                locationSource: 'GPS',
          ...
          console.warn("[seller] point de retrait indisponible au moment d'accepter la commande :", geoErr);
        }
      }
