...
// ✅ FIX : helper centralisé — écrit dans orders + seller_orders atomiquement
async function clientUpdateOrder(orderId: string, payload: Record<string, any>) {
  const batch = writeBatch(db);
  batch.update(doc(db, 'orders', orderId), payload);
  // Tenter de mettre à jour seller_orders (même ID) — ignoré si inexistant
  try { batch.update(doc(db, 'seller_orders', orderId), payload); } catch (_) {}
  return batch.commit();
}
