...
type FormData = {
  ...
  category: string; stock: string; unit: string; location: string;
  isOrganic: boolean; minOrder: string; tags: string;
};
...
export default function SellerProductsPage() {
  ...
  const handleSave = async () => {
    ...
    if (!form.stock || isNaN(Number(form.stock))) { setError('Stock invalide.'); return; }
    if (!user) return;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100 px-4 py-4 sticky top-14 z-30">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          ...
          <button
            onClick={openCreate}
            className="bg-green-600 hover:bg-green-700 text-white font-semibold px-4 py-2 rounded-xl text-sm flex items-center gap-2 transition"
          >
      ...
      {/* Notification succès */}
      {success && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 bg-green-600 text-white px-5 py-3 rounded-xl shadow-lg text-sm font-medium animate-slide-up">
          {success}
        </div>
      )}
              ...
              <div key={product.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <div className="aspect-video bg-gray-100 relative overflow-hidden">
                  ...
                  <div className="absolute top-2 left-2 flex gap-1 flex-wrap">
                    ...
                    {product.isOrganic && (
                      <span className="bg-emerald-100 text-emerald-700 text-xs px-2 py-0.5 rounded-full font-medium">🌿 Bio</span>
                    )}
                  </div>
                </div>
                ...
                <div className="p-4">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 text-xs font-medium py-2 rounded-xl border border-gray-200 hover:bg-gray-50 transition text-gray-700"
                    >
                      ✏️ Modifier
                    </button>
                    <button
                      ...
                      className="flex-1 text-xs font-medium py-2 rounded-xl border border-red-100 hover:bg-red-50 text-red-500 transition disabled:opacity-50"
                    >
          ...
          <div className="bg-white w-full sm:max-w-2xl sm:rounded-3xl rounded-t-3xl max-h-[92vh] flex flex-col">
            ...
            <div className="overflow-y-auto flex-1 px-6 py-4 space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap mb-2">
                  ...
                  <button
                    ...
                    className="w-20 h-20 rounded-xl border-2 border-dashed border-gray-300 hover:border-green-400 flex flex-col items-center justify-center text-gray-400 hover:text-green-500 transition text-xs"
                  >
              ...
              <Field label="Nom du produit *">
                <input
                  ...
                  placeholder="Ex: Tomates cerises bio de Casamance"
                  className={INPUT}
                />
              </Field>
              ...
              <Field label="Description">
                <textarea
                  ...
                  placeholder="Décrivez votre produit : fraîcheur, origine, qualité…"
                  rows={3}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="Commande minimum">
                  <input
                    type="number" min="1" value={form.minOrder}
              ...
              <Field label="Tags (séparés par des virgules)">
                <input
                  ...
                  placeholder="frais, local, bio, saison"
                  className={INPUT}
                />
              </Field>
              ...
              <label className="flex items-center gap-3 cursor-pointer py-2">
                <div
                  onClick={() => setField('isOrganic', !form.isOrganic)}
                  className={`w-11 h-6 rounded-full transition-colors ${form.isOrganic ? 'bg-green-500' : 'bg-gray-300'} relative`}
                >
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex gap-3 flex-shrink-0">
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="flex-1 py-3 rounded-xl bg-green-600 hover:bg-green-700 disabled:bg-green-400 text-white font-bold text-sm transition flex items-center justify-center gap-2"
              >
                {saving ? (
                  <><span className="inline-block w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> Sauvegarde…</>
                ) : (
                  editingId ? '✅ Modifier' : '✅ Publier le produit'
                )}
              </button>
            </div>
          </div>
