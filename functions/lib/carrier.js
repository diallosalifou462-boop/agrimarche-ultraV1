...
async function loadPrefixTable() {
    const now = Date.now();
    ...
    try {
        ...
        if ((data === null || data === void 0 ? void 0 : data.table) && Array.isArray(data.table) && data.table.length > 0) {
...
function normalizePhoneSN(raw) {
    ...
    if (digits.startsWith('+221'))
        ...
        return null; // indicatif étranger non géré ici
    digits = digits.replace(/^0+/, ''); // certains saisissent 077...
    if (!/^7\d{8}$/.test(digits))
        return null; // mobile SN = 9 chiffres commençant par 7
    return `+221${digits}`;
}
async function detectCarrier(phoneE164) {
    const national = phoneE164.replace('+221', '');
    const table = await loadPrefixTable();
    for (const entry of table) {
        if (national.startsWith(entry.prefix))
